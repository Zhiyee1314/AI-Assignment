...
Output:
  - Prints a table: number of features -> features used -> Accuracy per model
...
def main():
    ...
    for n in n_options:
        ...
        print(f"       ANN Acc: {row['ANN']:.4f} | SVM Acc: {row['SVM']:.4f} | KNN Acc: {row['KNN']:.4f}\n")
    ...
    # ---------------------------------------------------------------
    results_df["team_avg_acc"] = results_df[["ANN", "SVM", "KNN"]].mean(axis=1)
    best_row = results_df.loc[results_df["team_avg_acc"].idxmax()]
    best_n = int(best_row["n_features"])
    best_features = best_row["features"]
    best_avg = best_row["team_avg_acc"]

    print("===== SUMMARY =====")
    print(f"Best number of features (averaged across ANN/SVM/KNN): {best_n}")
    print(f"Selected features: {best_features}")
    print(f"Team-average Accuracy at this point: {best_avg:.4f}")

    # ---------------------------------------------------------------
    # 5. Plot: Number of Features vs Accuracy, ONE LINE PER MODEL
    # ---------------------------------------------------------------
    ...
    plt.plot(results_df["n_features"], results_df["team_avg_acc"], marker='D', label="Team Average",
              color="black", linewidth=2, linestyle='--')

    plt.axvline(x=best_n, color='red', linestyle=':', alpha=0.6)
    plt.scatter([best_n], [best_avg], color='red', s=150, zorder=5,
                label=f"Best: {best_n} features (avg Accuracy = {best_avg:.4f})")

    plt.title("Number of Features vs Accuracy (ANN, SVM, KNN)", fontsize=13, fontweight='bold')
