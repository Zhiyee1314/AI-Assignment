...
def load_and_clean_data():
    # Use only the verified original 768 patients so duplicate/synthetic rows
    # do not distort the feature distribution graphs.
    df = pd.read_csv(RAW_PATH).iloc[:768].copy()
    clean = df.copy()
