"""Standalone Streamlit page for the ANN ROC curve and AUC."""
...
PREDICTIONS_PATH = ROOT_DIR / "Data" / "ann_test_predictions.csv"
REQUIRED_COLUMNS = ["Actual", "Prediction", "Probability"]


st.set_page_config(
    page_title="ANN ROC Curve",
    page_icon="📈",
    layout="centered",
)
st.title("📈 ANN — ROC Curve & AUC")
st.caption(
    "Evaluated using the saved predictions from the same held-out test "
    "patients used by Ann_Model.py."
)
...
def compute_roc():
    if not PREDICTIONS_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/Ann_Model.py first."
        )
    ...
    if missing:
        raise ValueError(
            "ANN prediction CSV is missing columns: " + ", ".join(missing)
        )
    ...
    if values.isna().any().any():
        raise ValueError("ANN prediction CSV contains missing/non-numeric values.")
    if not values["Actual"].isin([0, 1]).all():
        raise ValueError("ANN Actual values must contain only 0 or 1.")
    if not values["Prediction"].isin([0, 1]).all():
        raise ValueError("ANN Prediction values must contain only 0 or 1.")
    if not values["Probability"].between(0, 1).all():
        raise ValueError("ANN Probability values must be between 0 and 1.")
    if values["Actual"].nunique() != 2:
        raise ValueError("ANN ROC-AUC requires both Outcome classes.")
...
try:
    ...
    axis.set_title("ROC Curve - ANN Model", fontsize=14, fontweight="bold")
